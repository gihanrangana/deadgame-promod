...
HumanOnFire()
{
	...
	for(;;){
		...
		{
			if(team=="allies"){
				PlayFXOnTag( level.fx_dg_run_attack, self, "J_knee_le" );
				PlayFXOnTag( level.fx_dg_run_attack, self, "J_knee_ri" );
			
			}else if(team=="axis"){
				PlayFXOnTag( level.fx_dg_run_defence, self, "J_knee_le" );
				PlayFXOnTag( level.fx_dg_run_defence, self, "J_knee_ri" );
        ...
        if(self.origin != ori)
        {
            if(team=="allies"){
                ...
                while (isAlive(self))
                    {
                        self detachall();
                        self setModel( "plr_carl_johnson" );
                        wait .1;
                    }
            }
        }
